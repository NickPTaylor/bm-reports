...
There is a fair bit of data here.  The first date in the data is `r format(min(demand_gw$date_time), "%F")` and the final date is `r format(max(demand_gw$date_time), "%F")` i.e. the final date is when I downloaded the data.  The data should be captured at apporoximately 5 mins intervals.  This is a quick santity check of this fact.
...
It is desirable to merge the API and grid watch data sets for each time window.
Since the times at which values are recorded in the two data sets are not synchronised, the `xts` package will be used to handle this:
